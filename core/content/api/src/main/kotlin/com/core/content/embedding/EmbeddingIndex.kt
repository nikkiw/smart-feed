package com.core.content.embedding
...
class EmbeddingIndex {
    ...
    private val embeddings = mutableListOf<Pair<String, FloatArray>>()
    ...
    fun get(id: String): FloatArray {
        return embeddings.first { it.first == id }.second
    }
    ...
    fun search(
        ...
        for (emb in embeddings) {
            ...
            val sim = dot(queryUnit, emb.second)
            if (pq.size < k) {
                // Heap not full, add current pair
                pq.add(emb.first to sim)
            } else {
