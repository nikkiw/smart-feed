...
constructor(
     ...
     * ```
     */
    override suspend fun onArticleVisited(articleId: ContentId): Embeddings? = withContext(ioDispatcher) {
        mutex.withLock {
            ...
            embeddingDao.getEmbeddings(articleId.value)?.unitEmbedding?.let { articleEmbeddings ->
                ...
                val userEmbedding =
                    if (userProfile == null) {
                        ...
                        val weightedEmb =
                            FloatArray(articleEmbeddings.size) { i ->
                        ...
                        // Formula: new_embedding =
                        // (old_embedding * visit_count + article_embedding * engagement) / (visit_count + 1)
                        val oldEmb = userProfile.embedding
                        val count = userProfile.visitsCount.toFloat()
                        val newEmb =
                            FloatArray(oldEmb.size) { i ->
     ...
     * ```
     */
    private fun normalizeTime(seconds: Long, minSeconds: Long = 1L, maxSeconds: Long = 600L): Float {
