...
fun ViewContext.ArticleRecommendationsView(
    ...
    val stateMessage = root.findViewById<TextView>(R.id.recommendationsStateMessage)
    val retry = root.findViewById<MaterialButton>(R.id.recommendationsRetry)
    retry.setOnClickListener { component.onRetry() }

    // Подписываемся на список рекомендаций
    component.model.subscribe(lifecycle) { model ->
        val list = (model.state as? ArticleRecommendationsComponent.State.Content)?.items.orEmpty()
        stateMessage.visibility = View.GONE
        retry.visibility = View.GONE
        when (val state = model.state) {
            ArticleRecommendationsComponent.State.Loading -> Unit
            is ArticleRecommendationsComponent.State.Content -> Unit
            ArticleRecommendationsComponent.State.Empty -> {
                stateMessage.setText(R.string.related_articles_empty)
