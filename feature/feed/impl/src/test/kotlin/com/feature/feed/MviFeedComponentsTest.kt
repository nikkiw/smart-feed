...
class MviFeedComponentsTest {
    ...
    @Test
    fun `article exposes local load failure without connectivity dependency`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `article recommendations expose valid local empty state`() =
        runTest(dispatcher) {
            val context = DecomposeTestUtils.createTestComponentContext()
    ...
    @Test
    fun `recommendation model distinguishes empty room while offline`() =
        runTest(dispatcher) {
            val context = DecomposeTestUtils.createTestComponentContext()
